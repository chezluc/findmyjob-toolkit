...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "GET" && url.pathname === "/health") {
    return json(response, 200, { ok: true, queue: queue.length, results: results.size });
  }
  ...
  if (request.method === "POST" && url.pathname === "/commands") {
    try {
      ...
      queue.push(command);
      results.set(command.id, { status: "queued", command });
  ...
  if (request.method === "GET" && url.pathname === "/ai-state") {
    ...
    queue.push(command);
    results.set(id, { status: "queued", command });
